...
json_path = os.path.join(
    "results_gb", "best_results_" + str(timestamp) + "_gradient_boost.json"
)
...
plt.legend()
plt.savefig(
    os.path.join("result_modeling", "GB PvA Scatter.png"),
    dpi=400,
    bbox_inches="tight",
)
plt.close()

with open("model_errs_and_coefs/gradient_boost_errors.txt", "w") as fout:
...
gb_weights = estimator.feature_importances_

coef_df = pd.DataFrame({"feature": features, "coef_magnitude": gb_weights})
coef_df = coef_df.sort_values(by="coef_magnitude", ascending=False)
coef_df.to_csv("model_errs_and_coefs/gb_plot_df.csv")
